...
const createElement = (tag) => {
  switch (tag) {
    case 'p': return document.createElement(tag)
    default: return document.createElementNS(ns, tag)
  }
}
...
export const setAttributeImmediately = (node, attrs = {}) => {
  Object.assign(node.attributes, attrs)   // the same as setAttribute
  if (node.tok) {
    Object.keys(attrs).forEach((key) => node.tok.setAttribute(getKebabCase(key), attrs[key]))
  }
}
