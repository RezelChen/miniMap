            ...
            fx.render = function() {
              ...
              if (fx.situation.play === true) {
                ...
                if (time > fx.situation.finish) {
                  ...
                  requestAnimationFrame(fx.render)
                }
              } else {
                //Fix bug: 暂停或停止时不再渲染
...
// Add event unbinder in the SVG namespace
  ...
  (function (w) {
    ...
    for (var x = 0; x < vendors.length && !window.requestAnimationFrame; ++x) {
      requestAnimationFrame = w[vendors[x] + 'RequestAnimationFrame']
      cancelAnimationFrame = w[vendors[x] + 'CancelAnimationFrame'] ||
    ...
    cancelAnimationFrame = cancelAnimationFrame || w.clearTimeout;

    //下面是自己实现的动画总控
    //实现一帧中只请求一次系统的的requestAnimationFrame，减少回调执行时的额外开销
    (function () {
      var map = {}; //存储所有的动画请求函数
      var seq = 0;  //函数序列号，控制在一帧内相同函数不重复执行
      var animationId;
      w.requestAnimationFrame = function (func) {
        if (map[func._animId]) {
          return;
        }
  ...
  var svgSetTimeout, svgClearTimeout;
  (function () {
    var taskQueue = [];
    var seq = 0;
    var isWaitOrRuning = false;
    svgSetTimeout = function (func, delay) {
      if (delay === 0) {
        func._taskSeq = (seq).toString();//字符串类型保证了不会和原生的timer id重复
        seq++;
        taskQueue.push(func);
        if (!isWaitOrRuning) {
          isWaitOrRuning = true;
          Promise.resolve().then(function () {
            var task = 0;
            while (taskQueue.length > 0) {//用这种方式遍历支持在task运行期间修改taskQueue
              ...
              if (typeof task === "function") {
                ...
                throw new Error(task.toString() + "is illegal parameter of svgSetTimeout");
              }
            }
            isWaitOrRuning = false;
          });
        }
        return func._taskSeq;
      }
      else {
    ...
    svgClearTimeout = function (id) {
      var toRmIndex;
      var isInQueue = taskQueue.some(function (item, index) {
        if (item._taskSeq === id) {
          toRmIndex = index;
          return true;
        }
